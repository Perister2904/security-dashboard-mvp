  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <div className="space-y-2">
          ...
          <div className="card p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className={`text-xl font-bold ${riskLevel.color} mb-1`}>{riskLevel.level}</div>
          ...
          <div className="card p-3 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-500">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <h4 className="font-bold text-xs text-yellow-900 mb-1.5">Financial Impact Estimate</h4>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <div className="text-xl font-bold text-yellow-800">${(estimateFinancialImpact() / 1000).toFixed(0)}K</div>
                    <div className="text-[8px] text-yellow-700">Potential loss</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-green-600">${((estimateFinancialImpact() * 0.6) / 1000).toFixed(0)}K</div>
                    <div className="text-[8px] text-gray-600">Remediation cost</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="card p-2">
            <h4 className="text-[10px] font-bold mb-1.5 flex items-center gap-1"><Shield className="w-3 h-3 text-blue-600" />Security Team Performance</h4>
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded p-2">
                ...
                <p className="text-[8px] text-green-800">Response in {currentSOCMetrics.meanTimeToRespond}min - {currentSOCMetrics.meanTimeToRespond < 15 ? 'above' : 'below'} industry standards</p>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded p-2">
                <div className="flex items-center gap-1 mb-1">
                  <TrendingUp className="w-4 h-4 text-blue-600" />
                  <span className="text-xs font-bold text-blue-700">{Math.round((currentSOCMetrics.incidentsResolved / currentSOCMetrics.incidentsCreated) * 100)}%</span>
                </div>
                <p className="text-[8px] text-blue-800">{currentSOCMetrics.incidentsResolved} of {currentSOCMetrics.incidentsCreated} incidents resolved (30d)</p>
              </div>
            </div>
          </div>

          {/* CISO Recommendations - Compact */}
          <div className="card p-2 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
            <h4 className="text-[10px] font-bold mb-1.5">CISO Recommendations</h4>
            <div className="space-y-1">
              <div className="flex gap-2 bg-white dark:bg-gray-800 p-1.5 rounded text-[9px]">
                <div className="w-4 h-4 bg-blue-600 text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold text-[8px]">1</div>
                <div><strong>Immediate:</strong> Approve emergency SQL injection patching</div>
              </div>
              <div className="flex gap-2 bg-white dark:bg-gray-800 p-1.5 rounded text-[9px]">
                <div className="w-4 h-4 bg-blue-600 text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold text-[8px]">2</div>
                <div><strong>30 days:</strong> Implement MFA for admin accounts (95% risk reduction)</div>
              </div>
              <div className="flex gap-2 bg-white dark:bg-gray-800 p-1.5 rounded text-[9px]">
                <div className="w-4 h-4 bg-blue-600 text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold text-[8px]">3</div>
        ...
        <div className="space-y-2">
          <h3 className="text-xs font-bold flex items-center gap-1"><AlertTriangle className="w-4 h-4 text-red-600" />Top Business Risks <span className="text-[8px] text-gray-500 font-normal">(Click for details)</span></h3>
          <div className="space-y-2">
            {riskPosture.criticalRisks.slice(0, 3).map((risk, i) => (
              <div key={i} className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded p-2 hover:border-red-400 cursor-pointer" onClick={() => setSelectedRisk(risk)}>
                <div className="flex items-start justify-between mb-1">
                  <div className="flex items-center gap-2 flex-1">
                    <span className="w-5 h-5 bg-red-600 text-white rounded-full flex items-center justify-center font-bold text-[10px]">{i + 1}</span>
                    <h4 className="font-bold text-xs text-red-900">{risk.title}</h4>
                  </div>
                ...
                <p className="text-[9px] text-red-800 mb-2 line-clamp-2">{risk.description}</p>
                <div className="bg-red-100 dark:bg-red-900/40 p-2 rounded">
                  <div className="flex items-start gap-1">
                    ...
                    <div>
                      <div className="font-semibold text-[9px] text-red-900 mb-0.5">Business Impact:</div>
                      <div className="text-[8px] text-red-800 line-clamp-2">{risk.businessImpact}</div>
                    </div>
                  </div>
                </div>
          ...
          <div className="card p-2">
            <h4 className="text-[10px] font-bold mb-1.5">Additional Metrics</h4>
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 rounded p-1.5 text-center">
                <div className="text-lg font-bold text-green-700">94%</div>
                <div className="text-[7px] text-green-600">Compliance Score</div>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 rounded p-1.5 text-center">
                <div className="text-lg font-bold text-blue-700">$2.5M</div>
                <div className="text-[7px] text-blue-600">Insurance Coverage</div>
              </div>
              <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 rounded p-1.5 text-center">
                <div className="text-lg font-bold text-purple-700">72%</div>
                <div className="text-[7px] text-purple-600">Security Posture</div>
              </div>
            </div>
          </div>

          {/* Revenue at Risk */}
          <div className="card p-2 bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500">
            <h4 className="text-[10px] font-bold text-orange-900 mb-1">Revenue at Risk Analysis</h4>
            <div className="space-y-1">
              <div className="flex justify-between text-[9px]">
      ...
      <div className="card p-2 bg-gray-50 dark:bg-gray-800 border-l-2 border-gray-400">
        <div className="flex items-center gap-2">
          <Mail className="w-3 h-3 text-gray-600" />
          <p className="text-[9px] text-gray-700 dark:text-gray-300">📊 Auto-generated from real-time data • Email CEO or download PDF anytime</p>
        </div>
      </div>
