...
exports.socService = {
    ...
    `);
        ...
        await (0, redis_1.cacheSet)(cacheKey, metrics, 60); // Cache for 1 minute
        return metrics;
    },
    async getMetricsHistory(days = 7) {
      ...
      ORDER BY metric_date DESC`, [days]);
        await (0, redis_1.cacheSet)(cacheKey, result.rows, 300); // Cache for 5 minutes
        return result.rows;
    },
    async getIncidents(filters) {
        ...
        const countResult = await database_1.default.query(`SELECT COUNT(*) as total FROM incidents ${whereClause}`, params);
    ...
    async getIncidentById(id) {
      ...
      WHERE i.id = $1`, [id]);
        if (result.rows.length === 0)
            return null;
        await (0, redis_1.cacheSet)(cacheKey, result.rows[0], 120); // Cache for 2 minutes
    ...
    async updateIncident(id, updates) {
        ...
        values.push(id);
        const result = await database_1.default.query(`UPDATE incidents 
      SET ${updateFields.join(', ')}, updated_at = NOW()
      WHERE id = $${paramIndex}
      RETURNING *`, values);
        // Invalidate cache
        await (0, redis_1.cacheInvalidatePattern)('soc:*');
        logger_1.default.info(`Incident ${id} updated`, { updates });
        return result.rows[0];
    },
    async getRecentEvents(limit = 50) {
      ...
      ORDER BY detected_at DESC
      LIMIT $1`, [limit]);
        await (0, redis_1.cacheSet)(cacheKey, result.rows, 30); // Cache for 30 seconds
        return result.rows;
    },
    ...
    async getTasks(incidentId) {
        ...
        const result = incidentId
            ? await database_1.default.query(query, [incidentId])
            : await database_1.default.query(query);
        await (0, redis_1.cacheSet)(cacheKey, result.rows, 120); // Cache for 2 minutes
        return result.rows;
    }
};
//# sourceMappingURL=soc.service.js.map