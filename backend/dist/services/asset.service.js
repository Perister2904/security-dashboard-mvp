...
exports.assetService = {
    async getAssets(filters) {
        ...
        const countResult = await database_1.default.query(`SELECT COUNT(*) as total FROM assets ${whereClause}`, params);
    ...
    `);
        ...
        const deptResult = await database_1.default.query(`
      SELECT 
        department,
        COUNT(*) as total,
    ...
    `);
        ...
        await (0, redis_1.cacheSet)(cacheKey, stats, 300); // Cache for 5 minutes
        return stats;
    },
    ...
    `);
        ...
        const distResult = await database_1.default.query(`
      SELECT 
        CASE 
    ...
    `);
        ...
        const topVulnResult = await database_1.default.query(`
      SELECT 
        id,
        name,
        type,
    ...
    `);
        ...
        await (0, redis_1.cacheSet)(cacheKey, posture, 300); // Cache for 5 minutes
        return posture;
    },
    ...
    `);
        ...
        const gaps = result.rows.map((row) => ({
            ...row,
            gaps: row.gaps.filter((g) => g !== null)
        }));
        await (0, redis_1.cacheSet)(cacheKey, gaps, 300); // Cache for 5 minutes
        return gaps;
    },
    async updateAsset(id, updates) {
        ...
        values.push(id);
        const result = await database_1.default.query(`UPDATE assets 
      ...
      RETURNING *`, values);
        ...
        logger_1.default.info(`Asset ${id} updated`, { updates });
        return result.rows[0];
    }
};
